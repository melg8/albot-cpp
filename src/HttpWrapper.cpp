...
std::shared_ptr<spdlog::logger> HttpWrapper::mLogger =
    spdlog::stdout_color_mt("HttpWrapper");
...
Poco::Net::NameValueCollection HttpWrapper::cookie =
    Poco::Net::NameValueCollection();
std::map<std::string, int> HttpWrapper::NAME_TO_NUMBER =
    std::map<std::string, int>();
std::vector<Character> HttpWrapper::characters = std::vector<Character>();
std::vector<HttpWrapper::Service> HttpWrapper::services =
    std::vector<HttpWrapper::Service>();
...
bool HttpWrapper::get_cached_game_version(int& version) {
  ...
  if (version_file.fail() || !version_file.is_open()) {
    mLogger->warn(
        "Failed to find local cache version! Assuming outdated cache.");
  } else {
    std::string temp;
...
bool HttpWrapper::get_game_version(int& version) {
  ...
  if (HttpWrapper::do_request("http://host.docker.internal/", raw_data)) {
    std::regex version_regex =
        std::regex("data\\.js\\?v=([0-9]+)\"", std::regex::extended);
    std::smatch sm;
...
void HttpWrapper::handleGameJson(MutableGameData& data) {
  ...
  for (auto& [key, value] : geo.items()) {
    ...
    if (value["x_lines"].is_array()) {
      ...
      for (const nlohmann::json& entry : spawns) {
        info->spawns.emplace_back(entry[0].get<double>(),
                                  entry[1].get<double>());
      }
  ...
  mLogger->info("Simplified Maps. Reduced line count by {}% ({} -> {})",
                std::trunc((initial - final) / initial * 10000) / 100, initial,
                final);
}
...
bool HttpWrapper::get_game_data() {
  ...
  if (get_cached_game_version(cached_version)) {
    if (cached_version == current_version ||
        config->at("servicesOnly").get<bool>()) {
      ...
      mLogger->info(
          "Cached game version does not match! Need: {} Have: {}. Fetching.",
          current_version, cached_version);
    }
    std::string raw_data;
    if (HttpWrapper::do_request("http://host.docker.internal/data.js",
                                raw_data)) {
...
bool HttpWrapper::do_post(
    const std::string& url,
    const std::string& args,
    const std::string& method,
    std::optional<std::reference_wrapper<std::string>> out,
    std::optional<std::reference_wrapper<std::vector<Poco::Net::HTTPCookie>>>
        cookies) {
  Poco::URI uri(url);
  ...
  mLogger->info("POST {} ({} {})", url, (size_t)response.getStatus(),
                response.getReason());
  if (out.has_value()) {
    mLogger->info("Storing {} bytes from response.",
                  Poco::StreamCopier::copyToString(rs, out.value().get()));
  }
...
bool HttpWrapper::do_head(
  ...
  mLogger->info("HEAD {} ({} {})", url, (size_t)response.getStatus(),
                response.getReason());
  if (cookies.has_value()) {
    response.getCookies(cookies.value().get());
  }
...
bool HttpWrapper::do_request(
  ...
  std::istream& rs = session.receiveResponse(response);
  mLogger->info("GET {} ({} {})", url, (size_t)response.getStatus(),
                response.getReason());
  if (out.has_value()) {
    mLogger->info("Storing {} bytes from response.",
                  Poco::StreamCopier::copyToString(rs, out.value().get()));
...
bool HttpWrapper::login() {
  ...
  if (envfile.is_open()) {
    ...
    // Attempt to connect to the server. Since we don't need to copy the
    // output, We pass a nullptr for the output. TODO: Get support for HTTP
    // HEADERS verb.
    if (HttpWrapper::get_game_version(HttpWrapper::online_version)) {
      mLogger->info("Successfully connected to server! {}",
                    HttpWrapper::online_version);
      std::string args = fmt::format(
          "{{\"email\":\"{}\",\"password\":\"{}\",\"only_login\":true}}", email,
          password);
      ...
      if (HttpWrapper::api_method("signup_or_login", args, out, cookies)) {
        for (size_t i = 0; i < cookies.size(); i++) {
          ...
          if (_cookie.getName() == "auth") {
            ...
            HttpWrapper::auth = HttpWrapper::session_cookie.substr(
                pos + 1, HttpWrapper::session_cookie.length());
...
bool HttpWrapper::get_characters() {
  ...
  if (HttpWrapper::api_method("servers_and_characters", "{}", out)) {
    ...
    // For some reason we don't just get an object, the API wraps it in an
    // array.
  ...
  try {
    ...
    if (service_jsons.is_array()) {
      ...
      for (const nlohmann::json& service_json : service_jsons) {
        HttpWrapper::services.emplace_back(
            service_json["name"].get<std::string>(),
            service_json["enabled"].get<bool>());
      }
    } else {
  ...
  try {
    ...
    if (char_jsons.is_array()) {
      ...
      for (size_t i = 0; i < char_jsons.size(); i++) {
        Character& character =
            HttpWrapper::characters.emplace_back(char_jsons[i]);
        HttpWrapper::NAME_TO_NUMBER.emplace(character.name, i);
        std::string macro_section = fmt::format("{}={}", character.name, i);
        std::transform(character.name.begin(), character.name.end(),
                       macro_section.begin(), ::toupper);
...
void from_json(const nlohmann::json& character_json, Character& character) {
  ...
  character.klass =
      ClassEnum::getClassEnum(character_json.value("type", "unknown"));
}
...
bool HttpWrapper::api_method(
    const std::string& method,
    const std::string& args,
    std::optional<std::reference_wrapper<std::string>> out,
    std::optional<std::reference_wrapper<std::vector<Poco::Net::HTTPCookie>>>
        cookies) {
  return HttpWrapper::do_post(
      fmt::format("http://host.docker.internal/api/{}", method), args, method,
      out, cookies);
}